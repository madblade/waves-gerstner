...
import {
    BackSide, BoxBufferGeometry, Mesh, ShaderMaterial, UniformsUtils, Vector3
} from 'three';
...
Sky.SkyShader = {
    ...
    fragmentShader: [
        ...
        '	Lin *= mix( vec3( 1.0 ), pow( abs(vSunE * ( ( betaRTheta + betaMTheta ) / ( vBetaR + vBetaM ) ) * Fex),' +
        '       vec3( 1.0 / 2.0 ) ), clamp( pow( abs(1.0 - dot( up, vSunDirection )), 5.0 ), 0.0, 1.0 ) );',
